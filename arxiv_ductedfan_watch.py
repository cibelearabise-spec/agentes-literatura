#!/usr/bin/env python3
"""
arxiv_ductedfan_watch.py
--------------------------
Agente de escopo fechado para o Fable 6 (veículo esférico com ventoinha
dutada): vigia o arXiv por preprints novos sobre aerodinâmica de
ventoinha dutada, VTOL e veículos de geometria não convencional
(esférica, tail-sitter). Não decide nada — só encontra e loga.

Junto com arxiv_plasma_watch.py, cobre as duas metades da arquitetura
do Fable 6: propulsor (plasma/Hall) e veículo (ventoinha dutada / corpo
esférico).

Uso:
  python3 arxiv_ductedfan_watch.py

Agendamento: mesmas opções dos outros agentes.
"""

import json
import os
import re
import urllib.parse
import urllib.request
from datetime import datetime, timezone

# ---------------------------------------------------------------------------
# CONFIGURAÇÃO
# ---------------------------------------------------------------------------

QUERY = (
    'abs:"ducted fan" OR abs:"ducted propeller" OR abs:"VTOL" '
    'OR abs:"tail-sitter" OR abs:"spherical vehicle" OR abs:"spherical drone" '
    'OR abs:"shrouded rotor"'
)

LOOKBACK_DAYS = 8
MAX_RESULTS = 50

BASE = "http://export.arxiv.org/api/query"
HERE = os.path.dirname(os.path.abspath(__file__))
STATE_FILE = os.path.join(HERE, "state_ductedfan.json")
LOG_FILE = os.path.join(HERE, "log_ductedfan.md")

ATOM_NS = "{http://www.w3.org/2005/Atom}"


# ---------------------------------------------------------------------------
# FUNÇÕES (mesma estrutura do arxiv_plasma_watch.py)
# ---------------------------------------------------------------------------

def _get(url, params):
    qs = urllib.parse.urlencode(params)
    with urllib.request.urlopen(f"{url}?{qs}", timeout=30) as resp:
        return resp.read()


def load_state():
    if os.path.exists(STATE_FILE):
        with open(STATE_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"seen_ids": []}


def save_state(state):
    with open(STATE_FILE, "w", encoding="utf-8") as f:
        json.dump(state, f, ensure_ascii=False, indent=2)


def search_arxiv():
    import xml.etree.ElementTree as ET

    params = {
        "search_query": QUERY,
        "sortBy": "submittedDate",
        "sortOrder": "descending",
        "max_results": MAX_RESULTS,
    }
    raw = _get(BASE, params)
    root = ET.fromstring(raw)

    entries = []
    cutoff = datetime.now(timezone.utc).timestamp() - LOOKBACK_DAYS * 86400

    for entry in root.findall(f"{ATOM_NS}entry"):
        arxiv_id_full = entry.find(f"{ATOM_NS}id").text.strip()
        arxiv_id = re.sub(r"^https?://arxiv\.org/abs/", "", arxiv_id_full)

        published_el = entry.find(f"{ATOM_NS}published")
        published = published_el.text.strip() if published_el is not None else ""
        try:
            pub_ts = datetime.strptime(published, "%Y-%m-%dT%H:%M:%SZ").replace(
                tzinfo=timezone.utc
            ).timestamp()
        except ValueError:
            pub_ts = 0

        if pub_ts < cutoff:
            continue

        title = entry.find(f"{ATOM_NS}title").text.strip().replace("\n", " ")
        summary = entry.find(f"{ATOM_NS}summary").text.strip().replace("\n", " ")
        authors = [
            a.find(f"{ATOM_NS}name").text
            for a in entry.findall(f"{ATOM_NS}author")
        ]
        authors_str = ", ".join(authors[:3]) + (" et al." if len(authors) > 3 else "")

        entries.append({
            "id": arxiv_id,
            "title": title,
            "authors": authors_str,
            "published": published[:10],
            "summary": summary[:280] + ("…" if len(summary) > 280 else ""),
            "url": f"https://arxiv.org/abs/{arxiv_id}",
        })
    return entries


def append_log(new_entries):
    ts = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d %H:%M")
    lines = [f"\n## Rodada — {ts}\n"]
    if not new_entries:
        lines.append("Nenhum preprint novo desde a última execução.\n")
    else:
        lines.append(f"{len(new_entries)} preprint(s) novo(s):\n")
        for e in new_entries:
            lines.append(
                f"- **{e['title']}**\n"
                f"  {e['authors']} — arXiv, {e['published']}\n"
                f"  {e['url']}\n"
                f"  _{e['summary']}_\n"
            )
    with open(LOG_FILE, "a", encoding="utf-8") as f:
        f.write("\n".join(lines))


def main():
    state = load_state()
    seen = set(state.get("seen_ids", []))

    entries = search_arxiv()
    new_entries = [e for e in entries if e["id"] not in seen]

    append_log(new_entries)

    seen.update(e["id"] for e in entries)
    state["seen_ids"] = sorted(seen)
    state["last_run"] = datetime.now(timezone.utc).isoformat()
    save_state(state)

    print(f"Rodada concluída: {len(new_entries)} preprint(s) novo(s) registrados em {LOG_FILE}")


if __name__ == "__main__":
    main()
